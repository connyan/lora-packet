...
const asJSONOption = cmdlineArgs.indexOf("--as-json");

function printUsageAndExit(): void {
  console.log("Usage:");
  console.log(
    "\tlora-packet-decode [--nwkkey <NwkSKey> --appkey <AppSKey> --cntmsb <fCntMSB> --as-json] --{hex|base64} <data>"
  );
  process.exit(1);
}
...
if (asJSONOption < 0) {
  ...
  const response: {} = packet.toObject();
